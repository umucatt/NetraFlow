...
当前本地数据使用 schema 1 存储。应用会迁移旧格式数据；如果检测到未来版本的 schema，会拒绝读取，避免旧版本误写新格式数据。存储写入会维护 `previous` 校验副本，当前数据损坏或缺失时可从通过校验的副本恢复。

开发环境的 runtime 和 userdata 与正式应用隔离，避免开发调试数据污染正式使用数据。当前本地数据本身不默认加密；手动导出和自动生成的快照文件可以按设置使用快照密码加密。
...
- 修改本地数据结构时，需要维护 schema 版本、迁移逻辑、future schema 拒绝策略和 `previous` 副本恢复路径。
