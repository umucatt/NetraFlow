...
test('global settings security and about copy match the current release text', () => {
  ...
  assert.equal(appSource.includes('小幽靈 - wuui'), true);
...
test('rollup prompt display disables selection without touching paste input', () => {
  const appSource = readProjectFile('src/App.tsx');
  const stylesSource = readProjectFile('src/styles.css');
  ...
  assert.match(
    stylesSource,
    /\.rollup-prompt-display,\s*\.rollup-prompt-display \*\s*\{[^}]*user-select: none;[^}]*-webkit-user-select: none;[^}]*cursor: default;[^}]*\}/s
  );
...
test('global search includes manual settings category without old result containers', () => {
  ...
  assert.match(searchCategoriesBlock, /grid-template-columns:\s*repeat\(5, minmax\(0, 1fr\)\)/);
...
test('local Noto fonts, emoji fallback, and about-page license copy stay wired', () => {
  ...
  assert.equal(appSource.includes('借助 ChatGPT 和 Codex、Windsurf'), true);
  assert.match(
    stylesSource,
    /\.about-netraflow__memo,\s*\.about-netraflow__memo \*\s*\{[^}]*user-select: none;[^}]*-webkit-user-select: none;[^}]*cursor: default;[^}]*\}/s
  );
  assert.match(
    stylesSource,
...
test('quick entry picker and example account aliases use current title and mark rules', () => {
  ...
  assert.match(
    stylesSource,
    /\.quick-single-entry-account-group \+ \.quick-single-entry-account-group\s*\{[^}]*border-top: 1px solid var\(--border-subtle, var\(--border-soft\)\);[^}]*padding-top: 14px;[^}]*margin-top: 14px;[^}]*\}/s
  );
