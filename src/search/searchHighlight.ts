import type { SearchHighlightStrength } from './searchTypes';
...
const getDigitRanges = (text: string, token: string) => {
  ...
  const ranges: SearchHighlightRange[] = [];
  let searchIndex = textDigits.indexOf(tokenDigits);

  while (searchIndex >= 0) {
    const start = digitPositions[searchIndex];
    const end = digitPositions[searchIndex + tokenDigits.length - 1];

    if (start !== undefined && end !== undefined) {
      ranges.push({ start, end: end + 1, strength: 'medium' });
    }

    searchIndex = textDigits.indexOf(tokenDigits, searchIndex + 1);
  }
